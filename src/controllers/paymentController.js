...
const getKeyId = () => {
  let key = (env.RAZORPAY_KEY_ID || process.env.RAZORPAY_KEY_ID || process.env['RAZORPAY_KEY_ID '] || '***REDACTED***');
  return key.split('\n')[0].trim();
};
const getKeySecret = () => {
  let secret = (env.RAZORPAY_KEY_SECRET || process.env.RAZORPAY_KEY_SECRET || process.env['RAZORPAY_KEY_SECRET '] || '***REDACTED***');
  return secret.split('\n')[0].trim();
};
